...
//            c.pay(price); // 총 상품 가격 = price
    ...
    public void payment() {
        ...
        while (true) {
            ...
            Map<Product, Integer> shopMap = shoppingCart.getShoppingCart();

            for(Product product : shopMap.keySet()){
                String name = product.getName();
                int value = shopMap.get(product);
                long price = 0;
                totalPrice += price;

                System.out.println("상품명 : " + name + " 수량 : " + value);
            }
            ...
            if (decision.equals("Y")) { //결제를 진행하는 경우
                ...
                long ep1 = 0; //결제로 적립된 포인트
                long remainingPoint = selectedCard.getPoint(); //해당 카드의 적립금

                if (PointDecision.equals("Y")) {
                    boolean pointEnough = false;

                    if (selectedCard.getPoint() + selectedCard.getCardBal() >= totalPrice) {
                        while (!pointEnough) {
                            ...
                            if (selectedCard.getPoint() >= usedPoint) {
                                ...
                                if (payBalance > 0) {
                                    ...
                                    selectedCard.addPoint(totalPrice);
                        ...
                        System.out.println("결제 금액 : " + totalPrice + "원 /현재 포인트 : " + selectedCard.getPoint() + "점, /현재 잔액 : " + selectedCard.getCardBal() + "원");
                        System.out.println("결제를 진행할 수 없습니다. 잔액이 부족합니다.");
                        return;

                    }
                } else if (PointDecision.equals("N")) { //포인트를 사용하지 않고 해당 카드의 잔액으로 결제
                    if (selectedCard.getCardBal() >= totalPrice) {
                ...
                //영수증 생성(수정 필요) -> 장바구니 객체에 담겨있는 모든 상품이 출력되도록 수정
                ...
                //영수증 미발행
                else if(ReceiptDecision.equals("N")){
                    System.out.println();
                    System.out.println("이용해 주셔서 감사합니다.");
                    System.out.println();
                    return;
